...
#load data
f1 = open('training_data_.csv', 'r', encoding='utf-8')
rdr = csv.reader(f1)
X_train = []
y_train = []
for line in rdr:
...
f2 = open('testing_data_.csv', 'r', encoding='utf-8')
rdr = csv.reader(f2)
X_test = []
y_test = []
for line in rdr:
...
f3 = open('validation_data_.csv', 'r', encoding='utf-8')
rdr = csv.reader(f3)
X_val = []
y_val = []
for line in rdr:
...
model.add(Conv1D(64, kernel_size=3, input_shape=X_train.shape[1:3], activation='relu'))
...
if wandb_use == True:
    ...
    wandb.config.input_dimension = input_dimension
